...
| 媒体查询 range 语法 | 104 | 104 | 16.4 | 16.4 | 63 | 20 |
...
³ `:has()` 与原生嵌套编译器都**不产出**；列在这里是因为它们经由你自己的 CSS 或组件库进入产物，发出去了就是发出去了。嵌套不靠猜就识别不了，所以不识别；`:has()` 识别得了，所以识别。见下文。

`:has()` 那一行要横着读。Chrome 2022 年就有了，Safari 更早，Firefox 直到 2023 年底才有——表里首尾差距最大的一行。也是唯一一行会出现「三个浏览器里看起来完全正确，第四个里整条规则不见了」的。
...
**谁产出它**：这里没有谁。它从你自己的 CSS 或组件库进来，原样穿过这一趟编译，然后被发出去。

**丢什么**：和任何读不懂的选择器一样，整条规则作废。它值得单列一行，不是因为损失有多大，而是因为支持是**什么时候**到的。`:has()` 是日常在用的选择器里最新的一个，四个引擎之间隔了好几年——Chrome 105 是 2022 年 8 月，Safari 15.4 还更早，Firefox 要到 2023 年 12 月的 121。一份在 Chrome 和 Safari 上都验收过的样式表，在旧一点的 Firefox 里可能正无声地少着若干条规则。

**怎么关**：这不是开关的事，是改写的事。把条件改成表达在元素自己身上：组件在有那个子元素时本来就会加的类名，或者在 `:has(*)` 其实想说「非空」的地方用 `:not(:empty)`。

路由不受影响。`:has()` 的参数说的是这个元素**装着**什么，而不是它画在哪张设计稿上，因此在判定规则归属画布之前就被摘掉了——`.page-card:has(.van-icon)` 是一张页面卡片，不管它里面装了什么。见[组件库适配](./libraries.zh-CN.md#一个选择器到底在说谁)。
...
**谁产出它**：写成 `(width >= 768px)` 或 `(768px <= width < 1280px)` 的 profile `query`，或原本就采用这套 Level 4 语法的业务 CSS。

**丢什么**：旧浏览器会把查询判为 false，整块规则都不再生效。若该块负责切换画布，默认画布以外的画布会完全消失。

**怎么关**：改写成等价的 `min-width` / `max-width`。独占上界可减去一个很小的量（如 `max-width: 1279.98px`），避免和下一段在边界重叠。
...
特性匹配前会排除注释和带引号的字符串。例如写在注释里的选择器、通过 `content` 输出的示例只是数据，不是浏览器要解析的语法，不能让兼容性质量门禁误失败；扫描会保持源码位置，因此样本仍准确指向原始样式表。
...
版本必须是点分数字版本（如 `14`、`13.4`），浏览器与版本之间可用空格、`@` 或 `>=`。`<` 和单独的 `>` 与「最低支持版本」语义相反，因此会被拒绝。若重复写了共享同一支持数据的别名（如 `chrome` 与 `android`），无论参数顺序如何，都会保留其中最老的版本。

**扫描的是「从此再没断过支持」的版本**，而不是第一个出现 `y` 的版本。个别特性发布后又被撤回过，取前者才是「从这个版本起可以放心」的答案。caniuse 写成 `13.4-13.7` 的区间是它自己不单独跟踪的版本段，审计取低端。

## 这不能代替真机

审计会把内置特性表中检测到的特性与指定浏览器版本比较。**报告为空只表示没有发现不受支持的已跟踪特性，不能认证每一处 CSS 语法。** 未跟踪的特性、不支持的值组合以及检测遗漏均不在此结论内。通过 API 调用时，还必须显式处理未知浏览器名。

它不能证明页面在那台设备上好看。真机测的是别的东西——渲染差异、软键盘顶起视口、地址栏收起时的高度跳变、WebView 的自定义行为。这些在[可选运行时](./runtime.zh-CN.md)里有对应的补丁，但那是另一个问题。

在一台当前设备上通过测试，并不能证明所有旧版本都支持。内置支持数据帮助识别已跟踪特性的版本风险；涉及渲染、特性组合或嵌入式 WebView 行为时，仍需测试有代表性的目标浏览器。两种证据不能互相替代。

## 相关

- [配置参考](./configuration.zh-CN.md) —— 上面每个开关的完整语义
- [命令行预览](./cli.zh-CN.md) —— `--targets` 和其余选项
- [可选运行时](./runtime.zh-CN.md) —— 软键盘、地址栏、WebView
- [发布与兼容性](./release.zh-CN.md) —— 产物格式与 Node 版本

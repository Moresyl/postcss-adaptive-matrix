...
There is no base configuration to restate when the defaults already fit: `withAtomicCss()` is the complete zero-config form. Atomic-only settings are also accepted directly — `withAtomicCss({ tokenPrefixes: '--size-' })` — while `withAtomicCss(base, options)` remains the form for wrapping a real configuration. Use an array only for several extra prefixes.

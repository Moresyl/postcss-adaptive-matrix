...
按路径判定画布是最容易配错、又最不容易发现的一项——写错了不报错，只是路由静默失效（见[构建工具集成](./integration.zh-CN.md#2-只有按路径匹配时才需要-from)）。`--from` 就是用来在上线前把路由试一遍的：
...
所以只有**至少一侧是编译器产出的公式**时才报。在默认数学策略下，每个生成长度都有函数外壳：双边界使用 `clamp()`，单边界使用 `min()` / `max()`，无边界或静态首选值则使用数值等价的 `calc(value)` 标记。检查据此识别生成结果，同时不会把作者有意写下的断点像素误报成接缝。显式兼容模式 `strategy: 'viewport'` 为了避开数学函数仍保持裸值，因此不在这项诊断范围内。一侧换算、另一侧没有，照样报——那正是一次跨画布，而且没有人比对过这两个数字。
...
误报方面：69 份一致性套件产物、本仓库示例工程，以及 10 个组件库的已发布样式表（Vant、NutUI、Varlet、antd-mobile 1x/2x、Taro UI、Element Plus、Ant Design、Arco Design、Quasar，合计约 3.2 MB CSS），`shrinks` 报告数均为 0。见[组件库适配](./libraries.zh-CN.md#这张表是核对过的)。

...
`P`、`D`、`L`、`U` 均为正数时，上式对视口宽度**单调不减**：区间内按正斜率跟随，区间外是常数。

这条性质有个直接用处：产物里任何「视口变宽、尺寸反而变小」的现象都不可能来自公式本身，只可能来自跨断点换了画布——也就是两张设计稿在同一个元素上给了互相矛盾的数字。命令行据此做接缝检查（见[断点处的倒退](./cli.md#断点处的倒退)），而且这个检查在这一类问题上是完备的：不会漏到别的宽度上去。

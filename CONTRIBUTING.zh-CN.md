...
`npm run docs:build` 会构建站点，并离线检查生成的本地搜索索引：双语 API 查询、语言加载回退，以及每个索引页面和章节锚点。这不等同于浏览器交互或视觉验收。默认部署前缀为 `/postcss-adaptive-matrix/`；部署到域名根路径时，在构建前设置 `DOCS_BASE=/`。修改前缀后需要重新构建，不要直接修改生成的 HTML。构建与自动搜索检查需使用同一环境变量。
